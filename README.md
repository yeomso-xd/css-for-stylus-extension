웹페이지 디자인을 바꿀 수 있는 확장 프로그램 Stylus([Chrome, Edge,](https://chromewebstore.google.com/detail/clngdbkpkpeebahjckkjfobafhncgmne?utm_source=item-share-cb) [Firefox](https://addons.mozilla.org/ko/firefox/addon/styl-us/))를 위한 CSS 저장소입니다. (물론 다른 확장 프로그램도 가능)

# 스타일

### 원광대학교 Waffle-i
|웹페이지|URL|파일 바로가기|
|---|---|---|
|대시보드 (시작 화면)|https://lxp.wku.ac.kr/home/view/std/dashBoard|[[🔗]](./lxp.wku.ac.kr/home/view/std/dashBoard.css)|
|강의 홈|https://lxp.wku.ac.kr/std/lecture|[[🔗]](./lxp.wku.ac.kr/std/lecture.css)|

### 대학 기초융합교육 컨소시엄
|웹페이지|URL|파일 바로가기|
|---|---|---|
|강의 홈|https://kfedu.or.kr/std/lecture|[[🔗]](./kfedu.or.kr/std/lecture.css)|
|강의수강|https://kfedu.or.kr/std/course|[[🔗]](./kfedu.or.kr/std/course.css)|
